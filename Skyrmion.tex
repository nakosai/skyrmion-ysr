...
%should existthat condensed matter holds They were also predicted to exist in fethe of the General context of skyrmions as tolopolical excitations: memory, manipulation, local creation via SP STM.
...
%ere we prove the existence of the new type of localized excitation on the skyrmion core we call  Sc-YSR state (alternative is skyrmion bound state (sbs)).  Show the main results upfront in the introduction. Both LDOS and SP-LDOS
%It became clear that skyrmions can exists not in but as a single and can be contollably manipulate. This creates a prospects of manipulation. 
%A drive towards heterostuctures e.g. to build the topological superconductors. This motivates us to consider a single skyrmion in the .
...
First and foremost, superconductivity could be useful in stabilizing the skyrmionic phase. The superconductivity, e.g. in non-centrosymmetric materials, could induce effective Dzyaloshinskii-Moriya interaction between the ferromagagnetic spins and, thus, stabilize the skyrmionic phase. A number of recent theoretical claims of a strong interaction between the magnetic impurities via tha superconducting host \cite{Yao2014,Menard2015} support that proposal. Using similar arguments, the superconductivity could mediate effective interaction between the skyrmions and, thus, affect their dynamics. 
...
%Skyrmions in MnSi \cite{Muhlbauer2009} and motion of skyrmions under current and temperature gradient  (topological Hall response). Skyrmion lattice observation in the seminconducting compound ${\text{Fe}}_{1\ensuremath{-}x}{\text{Co}}_{x}\text{Si}$ \cite{Munzer2010}. Topological Hall effect in MnSi .

%Near-room-temperature skyrmions in FeGe \cite{Yu2011}.

 

%Skyrmions in the ferrimagnetic material ${\mathrm{Cu}}_{2}{\mathrm{OSeO}}_{3}$ \cite{Adams2012}. 


%Skyrmions in Wiesendanger's group: atomic scale skyrmions..

%Skyrmions in a multiferroic insulating magnet \cite{Seki2012}
